...
class TestSafeExecutor(unittest.TestCase):
    ...
    def test_read_file_success(self):
        result = self.executor.read_file("test.txt")
        self.assertTrue(result.ok)
        self.assertEqual(result.content, "Hello Hermes")

    def test_path_traversal_blocked(self):
        # 嘗試讀取工作區外部檔案
        result = self.executor.read_file("../../start_hermes.py")
    ...
    def test_binary_file_blocked(self):
        # 嘗試讀取 .exe
        result = self.executor.read_file("danger.exe")
        self.assertFalse(result.ok)
    ...
    def test_truncation(self):
        ...
        (self.test_root / "large.txt").write_text(large_content)
        
        result = self.executor.read_file("large.txt")
        self.assertTrue(result.ok)
