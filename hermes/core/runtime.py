...
class HermesRuntime:
    ...
    def execute_task(self, task: str, **llm_config):
        ...
        try:
            ...
            # 強制引導模型使用 'path' 參數
            sys_prompt = (
                "You are Hermes Agent OS. Mode: READ_ONLY.\n"
            ...
            if plan:
                ...
                if spec:
                    ...
                    self.monitor.traces.append(RuntimeTrace("TOOL_RESULT", res.summary, {"ok": res.ok}))
                    ...
                    if res.ok:
                        ...
                        self.last_result.update({"status": "FAILED", "error": res.error})
                else:
